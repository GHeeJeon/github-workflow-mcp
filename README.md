...
| 도구 | 설명 |
|------|------|
| `create_branch` | GitHub 브랜치 생성 |
| `run_tests` | 테스트 실행 후 pass/fail + 출력 반환 |
| `commit_and_push` | 변경사항 커밋 + push |
| `create_pr` | Pull Request 생성 |
| `get_pr_review_summary` | PR 리뷰 코멘트 요약 |
| `run_workflow` | **AI 원클릭 자동화** — 코드 생성 → 테스트 → 수정 반복 → 커밋/push → PR |
